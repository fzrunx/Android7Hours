...
data class MenuItem(
    ...
    val route: (() -> Unit)? = null, // 클릭 시 이동할 경로
)
...
fun MypageMainScreen(
    ...
    val menuItems = listOf(
        MenuItem(Icons.Default.CalendarToday, "일정관리", route = { navController.navigate(MypageNavigationRoute.ManageTab) }),
        MenuItem(Icons.Default.Star, "즐겨찾기", route = { navController.navigate(MypageNavigationRoute.FavoriteTab) }),
        MenuItem(Icons.Default.Settings, "설정", route = { navController.navigate(MypageNavigationRoute.SettingTab) }),
        MenuItem(Icons.AutoMirrored.Filled.Help, "도움말", route = { }),
        MenuItem(Icons.Default.Shield, "개인정보 처리방침", route = { })
    )
    ...
    LazyColumn(
        ...
        // --- 통계 섹션 ---
        item {
            StatsSectionView(stats = MyPageDataSource.stats)
        }
